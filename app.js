(() => {
  ...
  function drawMiffy(ctx, x, y, facing, walkFrame, landed, earWiggle) {
    ...
    if (miffySpriteReady && miffySpriteImg) {
      ...
      const nH =
        miffySpriteImg.naturalHeight || miffySpriteImg.height || CHAR_H;
    ...
    const legStep = Math.floor(walkFrame / 3) % 4,
      lOff = [0, 1, 0, -1][legStep],
      rOff = [0, -1, 0, 1][legStep];
  ...
  function fillPX(ctx, x, y, w, h, col) {
    ctx.fillStyle = col;
    ctx.fillRect(
      snapPX(x),
      snapPX(y),
      Math.max(PX, snapPX(w)),
      Math.max(PX, snapPX(h)),
    );
  }
  ...
  function drawWaterAndPlatforms(ctx, W, H, todBlend, time) {
    ...
    PLATFORM_SEGMENTS.forEach((p) => {
      ...
      if (!submerged) {
        ...
        ctx.fillText(
          ROWS_CONFIG[p.tier].abbr.toUpperCase(),
          (xS + xE) / 2,
          yTop + PX + 1,
        );
        ctx.restore();
  ...
  function drawWorld(ctx, W, H, todBlend, time) {
    ...
    const skyH = H * GROUND_FRAC,
      bandH = Math.ceil(skyH / skyRows.length);
    skyRows.forEach((c, i) => {
      ctx.fillStyle = c;
    ...
    if (todBlend.sunAlpha > 0.05) {
      const sx = snapPX(W * 0.78),
        sy = snapPX(H * GROUND_FRAC * todBlend.sunY);
      // Halo
      ...
      for (let dy = -5; dy <= 5; dy++)
        for (let dx = -5; dx <= 5; dx++) {
          if (dx * dx + dy * dy <= 25) {
            ctx.fillStyle =
              dx === 0 && dy === 0 ? "#ffffff" : todBlend.sunColor;
            ctx.fillRect(sx + dx * PX, sy + dy * PX, PX, PX);
          }
        }
      ctx.globalAlpha = 1;
    ...
    while (hx < W) {
      const hillH = snapPX(
        Math.sin(hx * 0.012) * 20 + Math.cos(hx * 0.008) * 14 + 18,
      );
    ...
    if (todBlend.starAlpha > 0.3) {
      ...
      for (let fx = 0; fx < W; fx += PX * 3)
        ctx.fillRect(fx, gY - PX * 3, PX, PX * 2);
      ctx.globalAlpha = 1;
    }
  }
  ...
  function updateGame() {
    ...
    const platform = getPlatformAtX(miffy.x),
      platformY = platform.yTopFrac * gH;
    if (miffy.y >= platformY) {
      if (miffy.vy > 2 && !wasOnGround) {
        ...
        document.getElementById("hud-zone").textContent =
          ROWS_CONFIG[miffy.tier].full;
        document.getElementById("hud-zone").style.color =
          ROWS_CONFIG[miffy.tier].color;
    ...
    const wy = waterY(),
      underwaterNow = miffy.y > wy + 0.5,
      depth01 = Math.max(0, Math.min(1, (miffy.y - wy) / (gH * 0.25)));
    if (underwaterNow !== underwaterActive) {
      setUnderwater(underwaterNow, depth01);
      showHudAction(underwaterNow ? "underwater lfo" : "surface");
    } else if (Math.abs(depth01 - underwaterDepth01) > 0.06)
      setUnderwater(underwaterNow, depth01);
    ...
    if (Math.abs(miffy.vx) > 0.5 && miffy.onGround && footstepCooldown <= 0) {
      ...
      document.getElementById("hud-zone").textContent =
        ROWS_CONFIG[rowIdx].full;
      document.getElementById("hud-zone").style.color =
        ROWS_CONFIG[rowIdx].color;
    }
    ...
    orbs.forEach((orb) => {
      ...
      const orbX = orb.x * gW,
        orbY =
          groundY() * orb.yBase - Math.sin(gameTime * 0.04 + orb.phase) * 12,
        dx = miffy.x - orbX,
        dy = miffy.y - CHAR_H * 0.5 - orbY;
  ...
  function spawnJumpSplat(platY, rowIdx) {
    const gY = platY ?? groundY(),
      col = ROWS_CONFIG[rowIdx ?? miffy.tier ?? 0].color,
      particles = [];
